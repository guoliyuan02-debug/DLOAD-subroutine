...
%%  导入数据
res = xlsread('样本.xlsx');
total_samples = size(res, 1); % 样本总数 (100)
...
for fold = 1 : K
    ...
    % 2. 提取数据并转置 (特征为行，样本为列)
    P_train = res(trainIdx, 1:5)';
    T_train = res(trainIdx, 6)';
    M = size(P_train, 2);
    
    P_test = res(testIdx, 1:5)';
    T_test = res(testIdx, 6)';
    N = size(P_test, 2);
